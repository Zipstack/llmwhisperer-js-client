...
const client = new LLMWhispererClient({apiKey:process.env.LLMWHISPERER_API_KEY});
describe('LLMWhispererClient', () => {
  test('get_usage_info', async () => {
    ...
    expect(Object.keys(usage_info)).toEqual(expect.arrayContaining(expected_keys));
  });
  ...
  test.each(test_cases)(
    ...
    async (processing_mode, output_mode, input_file) => {
      ...
      const response = await client.whisper({processingMode:processing_mode, outputMode:output_mode, filePath:file_path, timeout: 200 });
  ...
  test.skip('whisper', () => {
    ...
    // );
    const response = client.whisper(
      'test_files/restaurant_invoice_photo.pdf',
      { timeout: 200, store_metadata_for_highlighting: true },
    );
  ...
  test.skip('whisper_status', () => {
    const response = client.whisper_status('7cfa5cbb|5f1d285a7cf18d203de7af1a1abb0a3a');
    console.info(response);
    expect(typeof response).toBe('object');
  });

  test.skip('whisper_retrieve', () => {
    const response = client.whisper_retrieve('7cfa5cbb|5f1d285a7cf18d203de7af1a1abb0a3a');
    console.info(response);
    expect(typeof response).toBe('object');
  });

  test.skip('whisper_highlight_data', () => {
    const response = client.highlight_data('9924d865|5f1d285a7cf18d203de7af1a1abb0a3a', 'Indiranagar');
    console.info(response);
    expect(typeof response).toBe('object');
  });
});