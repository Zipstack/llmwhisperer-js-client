...
class LLMWhispererClient {
    constructor({ baseUrl = '', apiKey = '', apiTimeout = 120, loggingLevel = '' } = {}) {
        const level = loggingLevel || process.env.LLMWHISPERER_LOGGING_LEVEL || 'debug';
    ...
    async whisper({
        ...
        if (!url && !filePath) {
            throw new LLMWhispererClientException('Either url or filePath must be provided', -1);
        }

        if (timeout < 0 || timeout > 200) {
            throw new LLMWhispererClientException('timeout must be between 0 and 200', -1);
        }
...
class LLMWhispererClientV2 {
    constructor({ baseUrl = '', apiKey = '', loggingLevel = '' } = {}) {
        ...
        this.baseUrl = baseUrl || process.env.LLMWHISPERER_BASE_URL_V2 || BASE_URL_V2;
        this.logger.debug(`base_url set to ${this.baseUrl}`);
            ...
            if (response.status === 202) {
                ...
                while (true) {
                    ...
                    if (whisperStatus.status === 'processing') {
                        ...
                        throw new LLMWhispererClientException('Whisper operation already delivered', -1);
                    } else if (whisperStatus.status === 'unknown') {
                        this.logger.debug('Status: unknown');
                        throw new LLMWhispererClientException('Whisper operation status unknown', -1);
